...
It is based on [reveal.js](https://github.com/hakimel/reveal.js/), a HTML-based presentation framework. For detailed info about reveal.js, see [the framework's own README](reveal.js/README.md).
...
3. Double-click `index.html`. Presentations will be played back in a loop.
...
1. Clone this repository: `git clone https://github.com/jetbrains/jetbrains-booth-presentation.git`

2. Open the project in WebStorm or another editor.

3. Edit slides contained in `index.html`. Use `img/screenshots/productname` to store images and videos. (See *How slides are organized* below to get an idea of the types of slides generally used.)

4. Commit and push your changes to the `master` branch (unless agreed otherwise).

### Folder structure

The root of the repository contains the following files and folders (amongst others):
...
The reveal.js folder is simply a copy of the https://github.com/hakimel/reveal.js repo. It has not been modified and can simply be updated for new versions. All JetBrains specific files are in the `css`, `img`, `js`, and `plugin` folders:

* `css` + `js` - CSS and scripts to change the look of the presentation
* `img` - images for the slides (see below)
* `plugin` - plugin functionality for reveal.js

### How slides are organized

Slides for all products are stored in a single file, `index.html`. Here's a simplified structure of the file:
